...
const AddCourse = () => {
  ...
  //       lectureOrder: lecture.lectureOrder ?? lectureIndex + 1, // ensure order
  //     }))
  //   }));
  ...
  //   formData.append("courseDescription", courseDescription); // If needed in backend
  //   formData.append("image", image);
  //   formData.append("chapters", JSON.stringify(structuredChapters)); // ✅ Send as string
  //   formData.append("educatorId", educatorId); // if required
  
  //   const token = localStorage.getItem("token"); // Or wherever you store the token (e.g., context)
          ...
          {showPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
              <div className="bg-white p-4 rounded-lg text-gray-700 relative w-full max-w-xl">
                ...
                <img
                  onClick={() => setShowPopup(false)} src={assets.cross_icon} alt="cross_icon" className="absolute top-2 right-2 cursor-pointer" /> 
                  </div> 
                  </div> )} 
                  </div> 
                  <button type="submit" className="self-center px-8 py-2 bg-green-500 text-white rounded-lg" > Create Course </button> </form> </div> 
                ); };
