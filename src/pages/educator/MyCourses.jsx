...
const MyCourses = () => {
  ...
  const fetchEducatorCourses = async () => {
    try {
      const response = await fetch('/api/courses');  // Adjust the endpoint as needed
      if (!response.ok) {
        throw new Error('Failed to fetch courses');
      }
      const data = await response.json();
      setCourses(data);  // Assuming data is the list of courses from the backend
    } catch (error) {
      ...
      <div className="w-full">
        ...
        <div>
          <table className="md:table-auto table-fixed w-full overflow-hidden border border-gray-200">
            <thead className="text-gray-900 border-b-2 border-gray-200 text-sm text-left">
              <tr>
                ...
                <tr key={index} className="border-b border-gray-500/20">
                  <td className="md:px-4 pl-2 md:pl-3 py-3 flex items-center space-x-3 truncate">
                    <img
                      ...
                      className="w-16"
                    />
                    <span className="truncate hidden md:block text-gray-600">
                      {course.courseTitle}
                    </span>
                  </td>
